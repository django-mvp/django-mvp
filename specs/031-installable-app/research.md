...
DaisyUI 5 declares each theme's colours in `node_modules/daisyui/theme/<name>.css` as `oklch()`
custom properties. Nothing at runtime can read them: the stylesheet is a static file and a
project's own theme is CSS the server never parses.

**Chosen**: a committed build artifact, `mvp/pwa/theme_colors.json`, mapping each prebuilt theme
name to its `--color-base-100` converted to a `#rrggbb` hex string. It is generated by a new
`invoke build-theme-colors` task that reads the 35 theme files and converts OKLCH → OKLab →
linear sRGB → gamma-encoded sRGB, clamped to the gamut, in plain Python. It is regenerated
whenever the daisyUI version changes, the same way the stylesheet is, and `invoke prerelease`
runs it.

- **Hex, not oklch**: manifest colour parsing differs between browsers and older engines, and
  Apple's `theme-color` handling predates `oklch`. Hex is safe everywhere.
- **base-100 for both colours**: the header and the page body both sit on `base-100` in the
  ...
  matches the page.
- **Consistency test without Node**: the committed stylesheet carries a `[data-theme=<name>]`
  block for every prebuilt theme. A test asserts that the JSON has exactly one entry for each of
  those names and that every value is a valid hex colour. CI needs no `node_modules` for it.
...
The resolved values (name, colours, worker URL) come from one function in `mvp/pwa/` that both
the head template (through the existing `mvp_config` context processor, or a small context
helper) and the manifest view call, so the two can never disagree.
