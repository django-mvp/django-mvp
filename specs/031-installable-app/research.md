...
"pwa": False,        # falsey → off; {"theme_color": "#…"} → on, with the required colour
```

The name and short name sit at the top level because they name the application everywhere: the
page title uses `site_name` too. The feature itself is turned on by setting `MVP_CONFIG["pwa"]`,
with no separate switch. The one colour it carries covers the manifest's theme and
...
**Chosen**: from `MVP_CONFIG["pwa"]["theme_color"]`, and nowhere else. The colour is required:
setting it is what turns the feature on.
...
## R7 — No startup checks

None. Checks for missing images and for an unmounted `mvp.urls` were built and removed at the
walkthrough. The images are produced at deployment, so a development checkout never has them,
and `mvp.urls` is mounted by every project that uses the shell. Pages keep rendering in both
cases, and the documentation covers the setup.
...
on its own line after the favicon links:

```django
      {% if mvp_config.pwa %}{% include "mvp/pwa/head.html" %}{% endif %}
```

With the feature off, that line renders as whitespace and nothing else. A golden-file test pins
the off-state head.
...
- `{% url 'mvp-pwa-manifest' as manifest_url %}` and the same for the worker. The `as` form
  yields nothing rather than raising when `mvp.urls` isn't mounted, so pages keep rendering.
- The manifest link, only when `manifest_url` resolved.
- `<meta name="theme-color">` from `mvp_config.pwa.theme_color`.
...
The manifest view computes the same values inline in Python, because it builds JSON rather than
rendering a template. The two routes exist in `mvp/urls.py` only while `MVP_CONFIG["pwa"]` is set.
