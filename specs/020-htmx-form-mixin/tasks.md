...
- [X] T020 [US2] Write test `test_missing_form_component_raises_improperly_configured`: htmx POST with invalid data on a view where `htmx_form_component` is explicitly set to `None` (overriding the default `"form.card"`) raises `ImproperlyConfigured` in `tests/test_views/test_htmx_form_mixin.py`
