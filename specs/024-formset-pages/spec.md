...
- ~~Roadmap item R12 keeps the rest of its scope. Only the form-rendering half of its first deliverable moves here. The list-page dependency, the unguarded module-level import in a view module, and the documented-but-absent form renderer setting stay with R12, whose framing needs the matching correction.~~
  ...
  **Refined 2026-08-05** (design review, SPEC-101): Roadmap item R12 keeps the rest of its scope, but this feature settles **the whole** of its deliverable about pages loading an undeclared template library, not half of it. The list page and the form page load the *same* distribution — `list_view.html` and `cotton/form/render.html` both load `crispy_forms_tags`, and no other third-party tag library appears on either path — so declaring it resolves both at once. The "reduced but working level of polish" framing goes with it, since crispy is no longer optional and there is nothing to degrade to. R12 keeps the unguarded module-level import in a view module, the documented-but-absent form renderer setting, and the check covering every optional dependency. The original text is struck rather than deleted, because it explains why R12 was scoped as it was.
