...
- [X] T002 Audit `mvp/views/__init__.py` — confirm `MVPTemplateView` and `MVPHomeView` are exported
...
- [X] T007 *(Removed)* No public aliases — `MVPTemplateView` and `MVPHomeView` are the only acceptable import names
...
## Phase 3: User Story 1 — MVPTemplateView Plain Layout-Aware Template (Priority: P1) 🎯 MVP

**Goal**: A developer can wire `MVPTemplateView` to a URL, create a template extending `page_view.html`, and the page renders inside the full layout with navigation chrome — with no model, form, or queryset.

**Independent Test**: Wire `MVPTemplateView.as_view(template_name="demo/about.html", page_title="About")` to `path("about/")` in the demo app, create the template, start the dev server, and visit `/about/` — full layout renders with "About" in the page header.

### Design & Implementation for User Story 1

- [X] T011 [US1] Add demo `MVPTemplateView` URL: add `path("about/", MVPTemplateView.as_view(template_name="demo/about.html", page_title="About Us", page_subtitle="Learn more", page_icon="info-circle", breadcrumbs=[{"text": "Home", "href": "/"}, {"text": "About"}]), name="about")` to `demo/urls.py`; import `MVPTemplateView` from `mvp.views`
...
- [X] T016 [P] [US1] Add unit tests in `tests/test_views/test_base.py` for `MVPTemplateView`: wire with `page_title="Test Title"`, call `get_context_data()`, assert `context["page"]["title"] == "Test Title"` and `context["page"]["class"].startswith("mvp-page")`
- [X] T017 [US1] Add pytest-playwright E2E test in `tests/test_views/test_base_e2e.py`: request `/about/` unauthenticated, assert `200 OK`, assert sidebar and navbar present, assert page title in heading

### Story 1 Validation (REQUIRED)

- [X] T018 [US1] Run `python manage.py check` — zero errors MUST be reported
- [X] T019 [US1] Run `pytest tests/test_views/` — all tests MUST pass

**Checkpoint**: `MVPTemplateView` wired, renders in layout, `page_title` visible, 405 for POST confirmed, unit and E2E tests passing.

---

## Phase 4: User Story 2 — MVPHomeView Guest/Dashboard Template Switch (Priority: P1)

**Goal**: A developer wires `MVPHomeView` at `/`, sets `landing_template_name` and `dashboard_template_name`, and the view serves the landing page to guests and the dashboard to authenticated users — same URL, no redirect, `ImproperlyConfigured` raised on misconfiguration.

**Independent Test**: Wire `MVPHomeView` at the demo root, create two templates, request the URL unauthenticated (landing content), then authenticated (dashboard content) — both `200 OK`, URL unchanged, and removing `dashboard_template_name` triggers `ImproperlyConfigured`.
...
- [X] T023 [US2] Update demo home URL in `demo/urls.py`: replace existing `MVPDemoView` home entry with `MVPHomeView.as_view(landing_template_name="demo/landing.html", dashboard_template_name="demo/dashboard.html")` for `path("", ...)` named `"home"`; import `MVPHomeView` from `mvp.views`
...
**Checkpoint**: `MVPHomeView` wired, template switching confirmed in browser, `ImproperlyConfigured` guard tested, E2E tests passing.
...
## Phase 6: User Story 4 — MVPTemplateView Full Layout Configuration Participation (Priority: P2)

**Goal**: `MVPTemplateView` honours all `PageMixin` layout attributes (`page_title`, `page_subtitle`, `page_icon`, `page_class`, `breadcrumbs`) — set via class attribute or `as_view()` kwargs — and the layout shell reflects the configured values.

**Independent Test**: Subclass `MVPTemplateView` with all `page_*` attributes set, render via test client, assert each value appears in the rendered HTML in the expected location.
...
- [X] T045 [P] Update `skills/django-mvp/SKILL.md`: document `MVPTemplateView` (import path, `template_name`, all `page_*` attributes, `as_view()` usage), `MVPHomeView` (import path, `landing_template_name`, `dashboard_template_name`, `ImproperlyConfigured` behaviour, `get_landing_context`/`get_dashboard_context` hooks), and the bundled templates (`mvp/landing.html`, `mvp/dashboard.html`)
...
Phase 1 (Audit)
  └── Phase 2 (Foundation — rename attrs, add guard, add aliases)
        ├── Phase 3 (US1: MVPTemplateView — demo about page)
        ├── Phase 4 (US2: MVPHomeView — demo home page + bundled templates)
...
|-------|----------|-----------|------------------------------|
| US1 — MVPTemplateView | P1 | Phase 2 | US2 (different files) |
| US2 — MVPHomeView | P1 | Phase 2 | US1 (different files) |
...
- T011–20 (US1: MVPTemplateView demo wiring + tests)
- T021–35 (US2: bundled templates + MVPHomeView demo + tests)
...
- Public API: only `MVPTemplateView` and `MVPHomeView` are exported; no shorter aliases
