...
| XIV — Browser tests are the exception | One added (T016). `data-set-theme` is markup this package has never shipped, so "the contract is the emitted attributes" does not hold for it — an attribute theme-change never binds passes every markup assertion and does nothing. A `localStorage` write read pre-paint on the next load is not expressible with the test client, which is the article's permitted case rather than its excluded one. | Pass |
