...
- Q: What variable load order should be enforced for overrides? -> A: Load _mvp_variables.scss before AdminLTE defaults so downstream values override default tokens.
...
- What happens when an override file omits variables that are used by the vendored theme? The system should fall back to default vendor-provided values without build failure.
...
- **FR-002**: System MUST support app-level variable overrides through a dedicated `_mvp_variables.scss` file.
...
- **FR-011**: Build ordering MUST load `_mvp_variables.scss` before vendored AdminLTE SCSS defaults so downstream apps can override default theme tokens without modifying vendored sources.
- **FR-012**: SCSS compilation MUST use django-compressor and django-libsass so the vendored AdminLTE sources and `_mvp_variables.scss` are processed through the project build pipeline.
...
- **Theme Override File**: The downstream app-owned `_mvp_variables.scss` input where developers define brand-specific variable values.
...
- Downstream applications already use the existing project style build process and can include one app-owned override variables file.
- One override file per downstream app is sufficient for the initial release; additional environment-specific override layers are out of scope.
- Teams prefer stable defaults and only override the subset of variables needed for branding.
- Documentation updates for the theming workflow will be delivered with this feature so teams know where customization belongs.
...
- AdminLTE SCSS token defaults are compatible with pre-import variable overrides from `_mvp_variables.scss`.
