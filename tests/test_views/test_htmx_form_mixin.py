...
# ---------------------------------------------------------------------------
# Phase 10: HtmxMixin trigger subsystem + _resolve_component() helper
# ---------------------------------------------------------------------------


def test_htmx_mixin_apply_triggers_standalone():
    """_apply_htmx_triggers() on a bare HtmxMixin (no HtmxFormMixin) sets HX-Trigger."""
    from django.views.generic import TemplateView
    ...
    view_cls = type(
        "StubMixinView",
        (HtmxMixin, TemplateView),
        {"template_name": "base.html", "htmx_trigger": "ping"},
    )
    view = view_cls()
    view.request = request
    view.kwargs = {}
    ...
    result = view._apply_htmx_triggers(HttpResponse())
    assert "HX-Trigger" in result
    assert "ping" in result["HX-Trigger"]


def test_resolve_component_client_header_priority():
    """_resolve_component() returns the allowlist component when a known alias is in the header."""
    from django.views.generic import TemplateView

    rf = RequestFactory()
    request = rf.get("/", HTTP_X_SUCCESS_COMPONENT="list")
    request.user = User()

    view_cls = type(
        "StubMixinView",
        (HtmxMixin, TemplateView),
        {"template_name": "base.html", "my_comp": "default.comp", "my_comps": (("list", "list.item"),)},
    )
    view = view_cls()
    view.request = request
    view.kwargs = {}
    view.args = []
    assert view._resolve_component("my_comp", "my_comps", "X-Success-Component") == "list.item"


def test_resolve_component_empty_allowlist_ignores_header():
    """_resolve_component() falls back to server default when the allowlist is empty."""
    from django.views.generic import TemplateView
    ...
    view_cls = type(
        "StubMixinView",
        (HtmxMixin, TemplateView),
        {"template_name": "base.html", "my_comp": "default.comp", "my_comps": ()},
    )
    view = view_cls()
    view.request = request
    view.kwargs = {}
    view.args = []
    assert view._resolve_component("my_comp", "my_comps", "X-Success-Component") == "default.comp"


def test_resolve_component_unknown_alias_falls_through():
    """_resolve_component() falls back to server default on an unknown alias."""
    from django.views.generic import TemplateView
    ...
    view_cls = type(
        "StubMixinView",
        (HtmxMixin, TemplateView),
        {"template_name": "base.html", "my_comp": "default.comp", "my_comps": (("detail", "detail.card"),)},
    )
    view = view_cls()
    view.request = request
    view.kwargs = {}
