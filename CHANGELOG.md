...
- **Full-page content, with `fill` on `<c-page>`.** A full-bleed map, or any of
  ...
  the chain was missing. Pages without `fill` are untouched: the shell's markup is
  unchanged and the rule is scoped so that it never applies to them. See
  [Layout: Full-page content](docs/layout.md#full-page-content).
